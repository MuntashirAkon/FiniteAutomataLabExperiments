...
class DFA {
    ...
    /**
     Transition table
     */
    std::vector< std::vector< state > > transition_table;
    /**
     Start state
     */
    ...
    /**
     Accepting states
     */
    const std::vector< state > accepting_states;
    /**
     Input symbols
     */
    const std::vector<input_symbol> input_symbols;
    /**
...
public:
     ...
     */
    DFA(int n_states, const std::vector<input_symbol> &input_symbols,
        state start_state, const std::vector<state> &accepting_states);
     ...
     */
    bool evaluate(const std::string &str, bool print_states = false);
    ...
    state tf(state q, input_symbol e){
        return (current_state = transition_table[q][get_index_by_input_symbol(e)]);
    }
};
